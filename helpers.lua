...
function help.spline_tool(start,length,mysplinefunc,myspeedmod)
		...
		xero.add_mod(start,length,function(x)
			...
			for i = 0,3 do
				...
				for splinetype,modpercent in pairs(receptortable) do
					...
					Player['Set'..splinetype..'Spline'](Player,which[splinetype..i],i,modpercent,0.001--[[or 0.0001]],-1)
				end
			end
			for num,note in mytable do
				if note[1] >= curbeat then
					...
					for splinetype,modpercent in pairs(resulttable) do
						if(which[splinetype..note[2]]<30)then 
							which[splinetype..note[2]]=which[splinetype..note[2]]+1
							Player['Set'..splinetype..'Spline'](Player,which[splinetype..note[2]],note[2],modpercent,(offset-10>0.001 and offset-10 or 0.001001),-1)
							which[splinetype..note[2]]=which[splinetype..note[2]]+1
